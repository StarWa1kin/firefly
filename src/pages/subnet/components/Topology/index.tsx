...
function format(data) {
  ...
  for (var i = c + 1; i < hi; i++) {
    if (l2[i].hasOwnProperty("children")) {
      if (l2[i - 1].hasOwnProperty("children") && l2[i - 1].children[l2[i - 1].children.length - 1].x - l2[i].children[0].x > -100) {
  ...
  for (var i = c - 1; i >= 0; i--) {
    if (l2[i].hasOwnProperty("children")) {
      if (l2[i + 1].hasOwnProperty("children") && l2[i + 1].children[0].x - l2[i].children[l2[i].children.length - 1].x < 100) {
...
function getLoction(num: number, x, index) {
  if (num % 2 === 0) {
    var c = num / 2 - index;
    x = x - 50 - 100 * c;
  } else {
...
function getMaxMin(maxmin: any[], x) {
  if (maxmin[0] < x) {
    maxmin[0] = x;
  }
  if (maxmin[1] > x) {
    ...
    series: [
      {
        ...
        zlevel: 5,
        draggable: false,
        coordinateSystem: "cartesian2d", //使用2-维的直角坐标系（也称笛卡尔坐标系）
        label: {
          show: true,
          color: "#000",
          position: "bottom",
